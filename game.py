...
def set_array_pos(array):
    ...
    for row in range(1, ROWS):
        for column in range(len(array[row])):
            if array[row][column] != BLANK:
                array[row][column].rect.y -= int(RADIUS * (2 - math.sqrt(3)) * row)
...
def end_screen(score, winorlose):
    ...
    message1Rect.center = (int(WIDTH / 2), 200)
    message2Rect = message2.get_rect()
    message2Rect.center = (int(WIDTH / 2), 300)
    message3Rect = message3.get_rect()
    message3Rect.center = (int(WIDTH / 2), 500)
...
def game_loop():
    ...
    while run:
        ...
        win.blit(rocket_copy, (int(rocket_x), int(rocket_y)))
